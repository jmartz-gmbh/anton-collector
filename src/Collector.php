...
class Collector
{
    ...
    public function run(string $identifier = 'all')
    {
        $collected = false;
        try {
            $anton = [];
            ...
            if ($projects) {
                foreach ($projects as $key => $project) {
                    if ($identifier == 'all' || $key == $identifier) {
                        exec('mkdir -p storage/logs/' . $key);
                        exec('rm -rf workspace/projects/' . $key);
            ...
            if (!$save && $collected) {
                ...
                echo 'Collect failed because Project (' . $identifier . ') unknown.' . PHP_EOL;
            } else {
